...
def draw_ui(frame, actions, detector, fps, is_active, joy_x=0.0, joy_y=0.0, h_list=None):
    h, w = frame.shape[:2]
    ...
    cv2.putText(frame, h_lbl, (x0, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.5, h_clr, 1)
    cv2.putText(frame, f"X:{joy_x:+.2f}", (x0, 110), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,255), 1)
    cv2.putText(frame, f"Y:{joy_y:+.2f}", (x0, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0,255,255), 1)
    dirs = []
    if joy_y > 0.15: dirs.append("UP")
    if joy_y < -0.15: dirs.append("DN")
    if joy_x < -0.15: dirs.append("L")
    if joy_x > 0.15: dirs.append("R")
    cv2.putText(frame, "-".join(dirs) if dirs else "CENTER", (x0, 160), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,255), 2)
    sens = getattr(detector, 'JOY_MAX_REACH', 0.06)
    cv2.putText(frame, f"Sens: {sens:.3f}", (x0, 190), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (180,180,180), 1)
    cv2.putText(frame, "+/- keys", (x0, 210), cv2.FONT_HERSHEY_SIMPLEX, 0.35, (100,100,100), 1)
    
    # --- DRAW HAND LANDMARKS ---
    if h_list:
        for hand in h_list:
            if hand["label"] == "Left":
                # Only draw yellow circle on left hand palm center
...
def main():
    try:
        ...
        sk = FullBodySkeleton(
            ...
            HandsConfig(max_num_hands=2), # Re-enabled for Start/Stop gestures and dual drawing
    ...
    print("IMCCA Started. Gestures: 10-Fingers=START, Index+Middle Only=STOP. ESC to quit.")
    prev = time.time()
    is_active = False # New state for capturing
    ...
    while True:
        try:
            ...
            draw_ui(fr, acts, det, fps, is_active, joy_x, joy_y, h_list)
            ...
            elif key == ord('r'):  # Reset neutral center
                det._neutral_x = None
                det._neutral_y = None
